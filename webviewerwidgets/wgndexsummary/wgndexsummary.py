...
class EnrichmentSet():
    ...
    def __init__(self, e_set_name):
        self.id_set_map = {}
        self.objects = set()
        dbpath = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'wgndexsummary.sqlite')
...
def run_query (hugos):
    ...
    if len(query_ids) > max_num_hugos:
        result = {'scores': None, 'coverage': None}
        return result
    matched_genes = {}
    for term in query_ids:
        matched_genes[term] = [term]
    standardized_search_terms = {'matched': matched_genes, 'unmatched':[]}
    result = e_data.get_scores_on_standarized_query_terms('wgndexsummary', standardized_search_terms, False)
