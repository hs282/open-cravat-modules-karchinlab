$.getScript('/result/widgetfile/wgndex_chd/cytoscape.js', function () {});

widgetGenerators['ndex_chd'] = {
    'variant': {
        'donterase': true,
        'width': 580, 
        'height': 580, 
        ...
        'function': function (parentDiv, row, tabName) {
            ...
            var v = widgetGenerators['ndex_chd'][tabName]['variables'];
            ...
            var networkids = getWidgetData('gene', 'ndex_chd', geneRow, 'networkid');
            var networknames = getWidgetData('gene', 'ndex_chd', geneRow, 'networkname');
            if (networkids == null) {
                v.nodata = true;
                return;
            }
            ...
            //Enrichment Div
            this.enrichmentDiv = getEl('div');
            this.enrichmentDiv.id = "ndex_chd_list_variant";
            this.enrichmentDiv.style.fontSize = '12px';
            this.enrichmentDiv.style.fontWeight = 'normal';
            this.enrichmentDiv.style.fontFamily = 'Roboto';
            this.enrichmentDiv.style.position = 'absolute';
            this.enrichmentDiv.style.zIndex = 1;
            this.enrichmentDiv.style.width = '96%';
            this.enrichmentDiv.style.height = '';
            addEl(parentDiv, this.enrichmentDiv);
            //Network Div
            this.networkDiv = getEl('div');
            this.networkDiv.id = "ndex_chd_canvas_variant";
            this.networkDiv.style.width = '100%';
            this.networkDiv.style.height = '100%';
            this.networkDiv.style.top = '10%';
            this.networkDiv.style.left = '10%';
            this.networkDiv.style.backgroundColor = 'rgb(250,250,250)';
            this.networkDiv.addEventListener('click', function (evt) {
                var options = document.getElementById('ndex_chd_options_variant');
                options.style.display = 'none';
                options.previousSibling.textContent = 
                    '> ' + self.selectedNetworkName;
            });
            addEl(parentDiv, this.networkDiv);
            // Legends div
            this.legendDiv = getEl('div');
            this.legendDiv.id = 'ndex_chd_legend_variant';
            this.legendDiv.style.position = 'absolute';
            this.legendDiv.style.left =' 20px';
            this.legendDiv.style.top = '70px';
            addEl(parentDiv, this.legendDiv);
            // Enrichment score
            ...
            // Network name area
            var div = getEl('div');
            div.id = 'ndex_chd_select_variant';
            div.className = 'ndex_chd_select_variant';
            ...
            addEl(this.enrichmentDiv, div);
            // Network options
            var optionsDiv = getEl('div');
            optionsDiv.id = 'ndex_chd_options_variant';
            ...
            for (var i = 0; i < self.data['body'].length; i++) {
                ...
                optionDiv.onmouseenter = function (evt) {
                    document.getElementById('ndex_chd_select_variant').textContent = 
                    ...
                    document.getElementById('ndex_chd_options_variant').focus();
                }
                optionDiv.onclick = function (evt) {
                    document.getElementById(
                            'ndex_chd_options_variant').style.display 
                            = 'none';
                    var networkUUID = evt.target.getAttribute('networkid');
                    self.selectedNetworkName = 
                        evt.target.getAttribute('networkname');
                    var selectDiv = document.getElementById(
                            'ndex_chd_select_variant');
            ...
            addEl(this.enrichmentDiv, optionsDiv)
				
            // Shows first network
            var option = optionsDiv.getElementsByClassName(
                    'ndex_chd_option_variant')[0];
            var networkUUID = option.getAttribute('networkid');
            var networkName = option.getAttribute('networkname');
            optionsDiv.style.display = 'none';
            var select = this.enrichmentDiv.getElementsByClassName('ndex_chd_select_variant')[0];
            ...
            function drawNetwork (networkUUID, asCXResponse) {
                ...
                var nodeIdToHugo = {};
                for (var i = 0; i < asCXResponse.length; i++) {
                    var item = asCXResponse[i];
                    var els = item['nodeAttributes'];
                    if (els != undefined) {
                        for (var i = 0; i < els.length; i++) {
                            ...
                            if (el['n'] == 'Label') {
                                var nodeId = el['po'];
                                var hugo = el['v'];
                                var uniprot = hugo.replace('_HUMAN', '');
                                var uniprotHugo = ndex_prot_hugo_dic[uniprot];
                                if (uniprotHugo != undefined) {
                                    nodeIdToHugo[nodeId] = uniprotHugo;
                                } else {
                                    nodeIdToHugo[nodeId] = hugo;
                                }
                            }
                        }
                        break;
                ...
                for (var i = 0; i < asCXResponse.length; i++) {
                    ...
                    if (els != undefined) {
                        for (var i = 0; i < els.length; i++) {
                            var el = els[i];
                            el['n'] = nodeIdToHugo[el['@id']];
                        }
                        break;
                    }
                }
                var networkInfo = enrichmentScores[networkUUID];
	...
	'gene': {
		...
		'function': function (parentDiv, row, tabName) {
			...
			$(parentDiv).empty();
			self.selectedNetworkName = '';
            var networkids = getWidgetData(tabName, 'ndex_chd', row, 'networkid');
            var networknames = getWidgetData(tabName, 'ndex_chd', row, 'networkname');
            var hugo = getWidgetData(tabName, 'base', row, 'hugo');
			if (networkids == null) {
				return 'No data';
			}
			...
			//Enrichment Div
			this.enrichmentDiv = getEl('div');
			this.enrichmentDiv.id = "ndex_chd_list_gene";
			this.enrichmentDiv.style.fontSize = '12px';
			this.enrichmentDiv.style.fontWeight = 'normal';
			this.enrichmentDiv.style.fontFamily = 'Roboto';
			this.enrichmentDiv.style.position = 'absolute';
			...
			//Network Div
			this.networkDiv = getEl('div');
			this.networkDiv.id = "ndex_chd_canvas_gene";
			this.networkDiv.style.width = '100%';
			this.networkDiv.style.height = '100%';
			this.networkDiv.style.top = '10%';
			this.networkDiv.style.left = '10%';
			this.networkDiv.style.backgroundColor = 'rgb(250,250,250)';
			this.networkDiv.addEventListener('click', function (evt) {
				var options = document.getElementById('ndex_chd_options_gene');
				options.style.display = 'none';
				options.previousSibling.textContent = 
					'> ' + self.selectedNetworkName;
			});
			...
			// Legends div
			this.legendDiv = getEl('div');
			this.legendDiv.id = 'ndex_chd_legend_gene';
			this.legendDiv.style.position = 'absolute';
			this.legendDiv.style.left =' 20px';
			this.legendDiv.style.top = '70px';
			addEl(parentDiv, this.legendDiv);
			...
			// Network name area
			var div = getEl('div');
			div.id = 'ndex_chd_select_gene';
			div.className = 'ndex_chd_select_gene';
			div.style.fontFamily = 'Roboto';
			div.style.fontSize = '12px';
			div.style.fontWeight = 'normal';
			div.style.cursor = 'pointer';
			...
			// Network options
			var optionsDiv = getEl('div');
			optionsDiv.id = 'ndex_chd_options_gene';
			optionsDiv.style.fontFamily = 'Roboto';
			optionsDiv.style.fontSize = '12px';
			optionsDiv.style.fontWeight = 'normal';
			optionsDiv.style.maxHeight = '400px';
			...
			for (var i = 0; i < self.data['body'].length; i++) {
				...
				var networkId = rowData.attrs[0].replace(/networkUUID:/g, '');
				var optionDiv = getEl('div');
				optionDiv.className = 'ndex_chd_option_gene';
				var networkName = rowData.cols[0];
				optionDiv.textContent = networkName;
				optionDiv.setAttribute('networkid', networkId);
				optionDiv.setAttribute('networkname', networkName);
				...
				optionDiv.style.paddingBottom = '3px';
				optionDiv.onmouseenter = function (evt) {
					document.getElementById('ndex_chd_select_gene').textContent = 
						'v ' + evt.target.getAttribute('networkname');
					var options = document.getElementsByClassName(
							'ndex_chd_options_gene');
					for (var j = 0; j < options.length; j++) {
					...
					document.getElementById('ndex_chd_options_gene').focus();
				}
				optionDiv.onclick = function (evt) {
					document.getElementById(
							'ndex_chd_options_gene').style.display 
							= 'none';
					var networkUUID = evt.target.getAttribute('networkid');
					self.selectedNetworkName = 
						evt.target.getAttribute('networkname');
					var selectDiv = document.getElementById(
							'ndex_chd_select_gene');
					selectDiv.textContent
						= '> ' + self.selectedNetworkName;
					selectDiv.style.border = '2px outset #dddddd';
					selectDiv.style.fontWeight = 'bold';
			...
			// Shows first network
			var option = optionsDiv.getElementsByClassName(
					'ndex_chd_option_gene')[0];
			var networkUUID = option.getAttribute('networkid');
			var networkName = option.getAttribute('networkname');
			optionsDiv.style.display = 'none';
			var select = this.enrichmentDiv.getElementsByClassName('ndex_chd_select_gene')[0];
			select.textContent = '> ' + networkName;
			select.style.border = '2px outset #dddddd';
			if (networkUUID != '') {
				var url = 'https://www.ndexbio.org/v2/network/' + networkUUID;
